...
// CONFIG
const INPUT_CSV = path.join(process.cwd(), 'IHME Sources', "Global, Indonesia, and Indonesia's province incidence of malaria.csv");
const OUTPUT_JSON = path.join(process.cwd(), 'src', 'lib', 'data', 'ihme', 'processed', 'malaria_incidence.json');
...
function main() {
    ...
    // Standard: Global -> "Global", Indonesia -> "IDN", others -> exact string (will correspond to UI labels)
    ...
    // Generate suggested regions list for regions.ts
    console.log("\n--- SUGGESTED REGIONS LIST FOR regions.ts ---");
    console.log("export const uiAvailableRegions = [");
    console.log('    "Global",');
